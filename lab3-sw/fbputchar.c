...
void horiz_line(int line_y);
void scroll_display_area(void);
...
void clear_screen() {
    for (int y = 0; y < fb_vinfo.yres; y++) {
        for (int x = 0; x < fb_vinfo.xres; x++) {
            unsigned int location = (x + fb_vinfo.xoffset) * (fb_vinfo.bits_per_pixel/8) +
                                    (y + fb_vinfo.yoffset) * fb_finfo.line_length;
            *(framebuffer + location) = 0; //Blue 
            *(framebuffer + location + 1) = 0; // Green
            *(framebuffer + location + 2) = 0;//Red 
            *(framebuffer + location + 3) = 0; // Unused
        }
    }
}
...
void horiz_line(int line_y) {
    for (int x = 0; x < fb_vinfo.xres; x++) {
        unsigned int location = (x + fb_vinfo.xoffset) * (fb_vinfo.bits_per_pixel/8) +
                                (line_y + fb_vinfo.yoffset) * fb_finfo.line_length;
        *(framebuffer + location) = 0; //blue 
        *(framebuffer + location + 1) = 0; // Green
        *(framebuffer + location + 2) = 255; //red 
        *(framebuffer + location + 3) = 0; // Unused
    }
}

void scroll_display_area() {
    int bytes_per_row = fb_vinfo.xres * (fb_vinfo.bits_per_pixel / 8);
    int scroll_height = FONT_HEIGHT; // Scroll by one line height
    char* start = framebuffer + scroll_height * bytes_per_row;
    size_t size = (fb_vinfo.yres - scroll_height) * bytes_per_row;
    
    memmove(framebuffer, start, size);
    // Clear the last line
    memset(framebuffer + size, 0, scroll_height * bytes_per_row);
}
...

{
    ...
    int cursor_height = FONT_HEIGHT; // Cursor height could be same as font height

    int x = col * FONT_WIDTH * 2; // X position based on column

    int y;

    for (y = row * FONT_HEIGHT * 2; y < (row * FONT_HEIGHT * 2 + cursor_height); y++) {

        unsigned int location = (x + fb_vinfo.xoffset) * (BITS_PER_PIXEL / 8) +

                                (y + fb_vinfo.yoffset) * fb_finfo.line_length;

        // Set cursor color (white here)

        *(framebuffer + location) = 255; //Red

        *(framebuffer + location + 1) = 255; // Green

        *(framebuffer + location + 2) = 255; // Blue

        *(framebuffer + location + 3) = 0; // Transparency (unused in this case)
...

{
    ...
    for (y = row * FONT_HEIGHT * 2; y < (row * FONT_HEIGHT * 2 + cursor_height); y++) {
        ...
        // Set the pixel color to the background color (assumed black here)

        *(framebuffer + location) = 0; // Red

        *(framebuffer + location + 1) = 0; // Green

        *(framebuffer + location + 2) = 0; // /blue
